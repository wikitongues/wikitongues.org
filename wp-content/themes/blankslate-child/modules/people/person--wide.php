<?php
// Portrait plus the full detail block. Rendered for every person a gallery
// returns, so nothing here can assume a populated picture or language list.
	...
	<aside class="wt_person--wide__meta">
		...
		<?php if ( $personal_languages ) : ?>
			...
			<ul>
				<?php
				/*
				 * Resolve each language by ID rather than setup_postdata(). This
				 * module renders inside render_gallery_items(), where $post is a
				 * function-local variable — so setup_postdata() never reached the
				 * global that the_permalink()/the_field() read from, and every name
				 * came back empty (leaving a row of bare commas). Resolving
				 * explicitly also avoids wp_reset_postdata() clobbering the
				 * gallery's own loop.
				 */
